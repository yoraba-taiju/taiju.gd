  ...
  where TParam: struct, IBullet
{
  ...
  [Export(PropertyHint.Range, "0,256,1")] private uint bulletCount_ = 64;
  [Export(PropertyHint.Range, "0,0.5,0.05")] protected float BulletResizeScale = 0.1f;
  ...
  public override void _Ready() {
    ...
    for (var i = 1; i <= 20; ++i) {
      multiMeshInstance_.SetLayerMaskValue(i, i == 11);
    }
  ...
  private void SpawnEnqueuedBullets(double integrateTime) {
    ...
    GD.PrintErr($"Failed to spawn {leftBullets} bullet. Full (of {bulletCount_} bullets).");
    spawnQueue_.Clear();
  }
  ...
  private void ProcessBullets(bool forward, double integrateTime) {
    ...
    var isCollisionEnabled = Sora.IsCollisionEnabled;
    for (var i = 0; i < bulletCount_; ++i) {
      ...
      var time = integrateTime - bullet.SpawnAt;
      var attitude = bullet.Param.AttitudeAt(time);
      var pos = attitude.Position;
      var angle = attitude.Velocity;
      var resp = isCollisionEnabled ? OnBulletMove(attitude) : Response.None;
      ...
      var scale = 1.0f + (float)Mathf.Sin(Mathf.DegToRad(time * 360.0f * 3.0f)) * BulletResizeScale;
      var trans = Transform2D.Identity
        .TranslatedLocal(pos)
        .RotatedLocal(angle.Angle())
        .ScaledLocal(new Vector2(scale, scale));
